...
var strToInt conversions.Converter[any] = func(v any) (any, error) {
...
var intToStr conversions.Converter[any] = func(v any) (any, error) {
	i := v.(int)
	return strconv.FormatInt(int64(i), 10), nil
}

var intToUint conversions.Converter[any] = func(v any) (any, error) {
	i := v.(int)
	if i >= 0 {
		return uint(i), nil
	}
...
var uintToFloat32 conversions.Converter[any] = func(v any) (any, error) {
	i := v.(uint)
...
var _ = Describe("Conversions", func() {
	...
	It("convert will fail if the type extractor returns an error", func() {
		...
		_, err = conversions.Convert("123", reflect.TypeOf(int(0)))
		Expect(err).To(HaveOccurred())
	})

	It("convert will fail if no conversion is found", func() {
		conversions, err := conversions.New(
	...
	It("can convert a same type to same type", func() {
		conversions, err := conversions.New(
			conversions.WithTypeExtractor(testTypeExtractor),
		)
		Expect(err).ToNot(HaveOccurred())

		converted, err := conversions.Convert("test", reflect.TypeOf(""))
		Expect(err).ToNot(HaveOccurred())
		Expect(converted).To(Equal("test"))
	})

	It("can convert directly", func() {
		conversions, err := conversions.New(
			conversions.WithTypeExtractor(testTypeExtractor),
		)
		Expect(err).ToNot(HaveOccurred())
		...
		converted, err := conversions.Convert("123", reflect.TypeOf(0))
		Expect(err).ToNot(HaveOccurred())
		Expect(converted).To(Equal(123))
	})

	It("can convert indirectly", func() {
		conversions, err := conversions.New(
			conversions.WithTypeExtractor(testTypeExtractor),
		)
		Expect(err).ToNot(HaveOccurred())
		...
		converted, err := conversions.Convert("123", reflect.TypeOf(uint(0)))
		Expect(err).ToNot(HaveOccurred())
		Expect(converted).To(Equal(uint(123)))
	})

	It("can convert indirectly with 1 intermediate step", func() {
		conversions, err := conversions.New(
			conversions.WithTypeExtractor(testTypeExtractor),
		)
		Expect(err).ToNot(HaveOccurred())
		...
		converted, err := conversions.Convert("123", reflect.TypeOf(float32(0)))
		Expect(err).ToNot(HaveOccurred())
		Expect(converted).To(Equal(float32(123)))
	})

	It("can convert indirectly with 2 intermediate steps", func() {
		conversions, err := conversions.New(
			conversions.WithTypeExtractor(testTypeExtractor),
		)
		Expect(err).ToNot(HaveOccurred())
		...
		conversions.AddConversion(reflect.TypeOf(0), reflect.TypeOf(uint(0)), intToUint)
		conversions.AddConversion(reflect.TypeOf(uint(0)), reflect.TypeOf(float32(0)), uintToFloat32)
		conversions.AddConversion(reflect.TypeOf(float32(0)), reflect.TypeOf(float64(0)), func(v any) (any, error) {
			return float64(v.(float32)), nil
		})

		converted, err := conversions.Convert("123", reflect.TypeOf(float64(0)))
		Expect(err).ToNot(HaveOccurred())
		Expect(converted).To(Equal(float64(123)))
	})

	It("will handle loops in the conversion graph", func() {
		...
		_, err = conversions.Convert("123", reflect.TypeOf(uint(0)))
		Expect(err).To(HaveOccurred())
	})

	It("will handle custom type", func() {
		conversions, err := conversions.New(
		...
		converted, err := conversions.Convert(StringWithMediaType{
			MediaType: "text/html",
			Value:     "test",
		}, "text/plain")
		Expect(err).ToNot(HaveOccurred())
